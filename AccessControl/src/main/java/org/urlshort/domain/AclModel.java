...
public class AclModel {
    ...
    public void createUser(Long id) {
        ...
        var rl = roleRep.findByName(defaultRole)
                .orElseThrow(() -> new RuntimeException("Incorrect default role"));
        userRep.save(new User(id, rl));
    }

    @Transactional
    public void setUserRole(Long id, String role) {
        var rl = roleRep.findByName(role)
                .orElseThrow(() -> new NullObjectException(Role.class, role));
        var user = userRep.findById(id)
                .orElseThrow(() -> new NullObjectException(User.class, id.toString()));
        user.setRole(rl);
    }
    ...
    public void addRoleForPath(String path, String role) {
        var newRole = roleRep.findByName(role)
                .orElseThrow(() -> new NullObjectException(Role.class, role));
        var newPath = pathRep.findByName(path)
                .orElseThrow(() -> new NullObjectException(Path.class, path));
